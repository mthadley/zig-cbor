...
fn cborSize(comptime T: type, value: T) usize {
    switch (@typeInfo(T)) {
        ...
        else => |type_info| @compileError("Type size not yet implemented: " ++ @tagName(type_info)),
    }
}
...
fn cborSizeStatic(comptime T: type) usize {
    switch (@typeInfo(T)) {
        ...
        else => |type_info| @compileError("Type size not statically known: " ++ @tagName(type_info)),
    }
}
